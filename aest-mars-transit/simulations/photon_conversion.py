...
v4 changes from v3 — the receiver-technology half of the Gen 1 update
-----------------------------------------------------------------------------
...
This round investigated the original proposal to close that gap via a
...
series-connected subcells):
  ...
  - Critically, efficiency is ROBUST to partial/off-center illumination
    ...
    This directly answers a question that had to be
    checked, not assumed: does the ADR-003 spot-matched design's LOW
...
relabeled SINGLEJUNCTION_*, for historical/comparison reference):
  ...
  - PV_EFF_MULTIJUNCTION_TARGET = 0.60 (near the literature's
    ...
    the higher-intensity window, which further research found does NOT
...
inconsistency in the original proposal that needed resolving before
writing any more code — see ADR-005.
""")
