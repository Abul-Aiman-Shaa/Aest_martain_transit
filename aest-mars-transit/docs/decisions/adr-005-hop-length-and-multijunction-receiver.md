...
A single proposed change bundled two distinct claims: shorten hub-to-hub
beaming distance (away from the original 10km, ADR-001) to be "much
closer... economical," AND raise PV receiver efficiency above 50% via
"modular" power beaming with "multiple intake ports" — while flagging
that the latter might reintroduce the beam-pointing complexity ADR-003
had just resolved. This ADR records the literature research and physics
done to evaluate both claims, and finds they are **independent questions
with independent answers**, not one architectural change.
...
"multiple intake ports" concept maps onto multiple **subcells inside one
receiver**, not multiple **transmitters** — this closes the efficiency
gap with *zero* new beam-pointing complexity, directly answering the
original pointing-complexity concern.
...
- **Hub capital cost gets WORSE as hops shorten, not better.**
  ...
  at 0.5km (`corridor_economics.py` [2]). **The original "it needs to
  ...
  plainly, even though it complicates the original proposal.
...
- **A (rejected): literal multi-beam / multiple independently-tracked
  ...
  reintroduces exactly the pointing complexity already flagged as a
...
- **A (rejected): shortest hop tested (0.5-1km), maximizing dust-
    ...
    uneconomical on the same "be rigorous about economics" grounds
    already invoked.
...
- **Bad, stated plainly:** total hub/reactor capital for a fixed
  ...
  class units per 100km) — a real, non-trivial cost that the original
  "economical" framing did not anticipate would cut the other way on
  the capital axis. This is exactly the kind of finding that needs to
...
- `simulations/beam_tracking_sim.py` v2 (intensity-vs-tracking-margin
  check that resolves the pointing-complexity worry)
